...
Reservation.create = (newReservation, result) => {
    sql.query('INSERT INTO reservations SET ?', newReservation, (err, res) => {
        ...
        console.log('Reservation créé :', { id: res.insertId, ...newReservation });
...
Reservation.update = (id, reservation, result) => {
    sql.query(
        'UPDATE reservations SET created_at = ?, date_start = ? , duration = ?, nombre_personnes = ?, total = ?, id_cottages = ?  WHERE id = ?',
        [reservation.created_at, reservation.date_start, reservation.duration, reservation.nombre_personnes, reservation.total, reservation.id_cottages, reservation.id_users, id],
        (err, res) => {
            if (err) {
                console.log('Erreur :', err);
                result(err, null);
